  ...
  <section class="dashboard-user-stats" aria-label="文案用户用量统计">
    ...
    <div class="dashboard-ranking-grid">
      <article v-for="ranking in rankings" :key="ranking.key" class="card dashboard-ranking-card">
        ...
        <ol v-else>
          <li v-for="(item, index) in ranking.ranking" :key="`${item.name}-${index}`"><span>{{ index + 1 }}</span><strong :title="item.name">{{ item.name }}</strong><b>{{ formatMillions(item.usage) }} <em>百万 Tokens</em></b></li>
        </ol>
      </article>
    </div>
...
const periods = computed(() => [
  { key: 'today', label: '当天使用', ...props.periodStats.today, usage: formatMillions(props.periodStats.today.usage) },
  { key: 'week', label: '当周使用', ...props.periodStats.week, usage: formatMillions(props.periodStats.week.usage) },
  { key: 'month', label: '当月使用', ...props.periodStats.month, usage: formatMillions(props.periodStats.month.usage) }
])
