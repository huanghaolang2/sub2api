...
import { formatUsageBoardTokens, usageBoardTokenUnit, UsageBoardChartType, UsageBoardChoiceKind, UsageBoardLoadState, UsageBoardValidation } from '@/utils/usageBoard'
  ...
  <section class="usage-board min-w-0 space-y-6" data-testid="usage-board" :aria-label="t('usageBoard.title')">
    ...
    <div v-if="data" class="card overflow-hidden" data-testid="board-table">
      ...
      <DataTable :columns="columns" :data="data.rows" :row-key="rowKey" :server-side-sort="true" default-sort-key="total_tokens" :default-sort-order="sortOrder" @sort="onSort">
        ...
        <template #cell-total_tokens="{ row }"><span class="inline-flex items-center justify-end gap-2 tabular-nums" :data-state="row.data_state"><span>{{ formatUsageBoardTokens(row.total_tokens) }} {{ usageBoardTokenUnit(row.total_tokens) }} Tokens</span><span v-if="row.data_state === UsageBoardDataState.MISSING" class="rounded border border-dashed border-gray-300 px-1.5 py-0.5 text-xs text-gray-400 dark:border-dark-600 dark:text-dark-400">{{ t('usageBoard.noData') }}</span></span></template>
      </DataTable>
