...
describe('feature route guard', () => {
  ...
  beforeEach(() => {
    vi.stubEnv('VITE_USER_FEATURE_VISIBILITY', 'visible')
...
describe('subscription route guard (opt-out flag)', () => {
  ...
  it('keeps subscriptions hidden when release build visibility is disabled', async () => {
    vi.stubEnv('VITE_USER_FEATURE_VISIBILITY', 'hidden')
    appStore.cachedPublicSettings = { subscription_enabled: true }
    const { navigation, next } = runGuard({ requiresSubscription: true }, '/subscriptions')
    await navigation
