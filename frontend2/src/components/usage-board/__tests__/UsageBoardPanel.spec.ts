...
function render(scope = UsageBoardScope.SELF) {
  return mount(UsageBoardPanel, { props: { scope }, global: { plugins: [createI18n({ legacy: false, locale: 'zh', messages: { zh } })] } })
}
describe('UsageBoardPanel', () => {
  ...
  it('renders a real chart above the table and distinguishes observed zero from missing in both chart types', async () => {
    ...
    expect(table.findAll('tbody tr[data-state="observed"]')).toHaveLength(2)
    expect(table.get('tbody tr[data-state="missing"]').text()).toMatch(/0\s*无数据/)
    await wrapper.get('[data-testid="board-chart-period-1"]').trigger('focus')
    expect(wrapper.get('[data-testid="board-chart-tooltip"]').text()).toContain('0 tokens')
    ...
    expect(wrapper.get('[data-testid="board-chart-tooltip"]').text()).toContain('0 · 无数据')
  ...
  it('applies multi-select filters and sorting in both permission scopes', async () => {
    for (const scope of [UsageBoardScope.SELF, UsageBoardScope.ADMIN]) {
      ...
      for (const label of ['项目接口', '另一个接口', 'G1']) {
        const option = wrapper.findAll('label').find((item) => item.text() === label)
        if (!option) throw new Error(`Missing choice: ${label}`)
        await option.get('input[type=checkbox]').setValue(true)
      }
      await flushPromises()
      ...
      expect(wrapper.get('[data-testid="board-token-sort"]').element.parentElement?.getAttribute('aria-sort')).toBe('ascending')
  ...
  it('keeps empty periods visible without inventing an API key', async () => {
    ...
    expect(wrapper.findAll('tbody tr[data-state="missing"]')).toHaveLength(3)
